...
def test_init_ollama_no_attr(mocker):
    ...
    with patch("llm.llm_client.ollama", mock_ollama):
...
def test_openai_completion_non_streaming(mock_env_openai, messages, mocker):
    ...
    with patch("llm.llm_client.OpenAI", return_value=mock_openai_instance):
...
def test_ollama_completion_non_streaming(mocker, messages):
    # Mock ollama and ensure `_ollama_completion` returns a known value
    mock_ollama = mocker.MagicMock()
...
def test_ollama_completion_stream(mocker, messages):
    mock_ollama = mocker.MagicMock()
    # Simulate streaming by returning an iterable of tokens
